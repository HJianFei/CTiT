...
//        values.put(COLUMN_CLEAN_SESSION, conOpts.isCleanSession() ? 1 : 0); //convert boolean to int and then put in values
        //是否保存session 0 表示不保存，1 表示保存
        values.put(COLUMN_CLEAN_SESSION, 0); //convert boolean to int and then put in values
...
//            opts.setCleanSession(cleanSession);
            ...
            opts.setKeepAliveInterval(ActivityConstants.defaultKeepAlive);
            opts.setConnectionTimeout(ActivityConstants.defaultTimeOut);
//            opts.setPassword(password != null ? password.toCharArray() : null);
//            opts.setUserName(username);

//            if (topic != null) {
//                opts.setWill(topic, message.getBytes(), qos, true);
//            }

            //now create the connection object
            connection = Connection.createConnection(clientID, host, port, context);
